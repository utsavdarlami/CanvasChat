...
- [Frontend coordinate system](../llm_distribution_ui/docs/coordinate-system.md)
- [Backend chat coordinate contract](../llm_distribution_ui/docs/BACKEND_CHAT_COORDINATE_CONTRACT.md)
- [Display-local viewport model](../llm_distribution_ui/docs/DISPLAY_LOCAL_VIEWPORT_MODEL.md)
...
|-------|-----------|--------|-------|----------------|
| **Abstract** | Unitless floats from the backend layout algorithm or JSON upload | Backend / data file | Backend generates, frontend scales on first load | `-3.2`, `0.0`, `1.5` |
...
2. `applyDeltaRemovedNodes` — removes nodes and orphaned links
3. `applyDeltaAddedNodes` — inserts new nodes with the local display ID
4. `applyDeltaUpdatedLinks` — patches link properties
...
### Abstract → Canvas (initial load only)

```
canvas_x = ((abstract_x - domain_min_x) / domain_range_x) * viewport_range_x + padding
canvas_y = ((abstract_y - domain_min_y) / domain_range_y) * viewport_range_y + padding
```

**Source**: `coordinateScaling.ts:scaleValue()` (line 79)

This conversion happens once at graph upload via `useEntityFlowNodes`. After
scaling, the abstract space is discarded and all operations use canvas
coordinates.
...
5. After initial abstract→canvas scaling, never re-scale node coordinates.
